...
const Welcome = ({ onComplete }) => {
  ...
  reactExports.useEffect(() => {
    ...
    const checkPinnedStatus = async () => {
      if (typeof chrome !== "undefined" && chrome.action && chrome.action.getUserSettings) {
    ...
    checkPinnedStatus();
    const pinInterval = setInterval(checkPinnedStatus, 1e3);
    if (!checkInstallation()) {
      const installPoll = setInterval(() => {
        if (checkInstallation()) clearInterval(installPoll);
      }, 500);
      setTimeout(() => clearInterval(installPoll), 5e3);
    }
    let messageListener = null;
    if (typeof chrome !== "undefined" && chrome.runtime && chrome.runtime.onMessage) {
      messageListener = (message) => {
        if (message.action === "SIDEPANEL_OPENED" && (activeSection === "setup" || window.location.pathname.includes("install"))) {
