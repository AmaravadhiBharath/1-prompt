...
(function() {
  ...
  class SelectorRegistryService {
    ...
    async getRegistry() {
      ...
      if (cached && Date.now() - cached.lastUpdated < CACHE_TTL) {
        ...
        if (Date.now() - cached.lastUpdated > CACHE_TTL * 0.5) {
          this.fetchFromBackend().catch((err) => console.warn("[SelectorRegistry] Background refresh failed", err));
        }
        return cached;
      }
      try {
    ...
    formattedBundledDefaults() {
      return {
        ...
        platforms: {
          "chatgpt.com": {
            promptSelectors: [".whitespace-pre-wrap", "div[data-message-author-role='user']", "article div.flex-col.gap-1"],
            excludeSelectors: [".sr-only", "button", "svg"]
          },
          "claude.ai": {
            promptSelectors: [".font-user-message", "div[data-test-id='user-message']"],
            excludeSelectors: []
          },
          "deepseek.com": {
      ...
      if (prompts.length === 0) {
        ...
        allDivs.forEach((div, idx) => {
          if (!div.querySelector(".ds-markdown") && !div.closest(".ds-markdown")) {
            ...
            if (content && content.length > 10 && content.length < 2e3 && !seen.has(content)) {
  ...
  class GenericAdapter extends BaseAdapter {
    ...
    extractMessages(root) {
      ...
      while (node = walker.nextNode()) {
        ...
        if (lowerText.startsWith("here is") || lowerText.startsWith("sure, i can") || lowerText.startsWith("i have") || lowerText.startsWith("certainly") || lowerText.startsWith("of course") || lowerText.startsWith("here's a")) continue;
        if (text.length > 200) {
        ...
        if (lowerText.includes("the cow, dog, and") || lowerText.includes("yellow frog") || lowerText.includes("calm problem-solving") || lowerText.includes("sahai turns") || lowerText.includes("scattered ai conversations") || lowerText.includes("actionable workflow") || lowerText.includes("work with ai with an edge") || lowerText.includes("generates summaries of all prompts") || lowerText.includes("no duplicates") || lowerText.includes("cancel out the conflict") || /^(\d+\.|-|\*|•)?\s*(the image shows|image of|this image depicts)/i.test(text)) continue;
  ...
  function getScrollConfig(platformName2) {
    ...
    if (config2) {
      console.log(`[1prompt] Platform: ${config2.name} (${mode.toUpperCase()} mode)`);
      return config2;
    }
  ...
  function findActiveInput() {
    ...
    const textarea = document.querySelector('textarea:not([type="hidden"])');
    if (textarea && textarea.offsetParent !== null) return textarea;
  ...
  async function scrollConversation() {
    ...
    console.log(`[SahAI] Config: top=${config2.topAttempts}, bottom=${config2.bottomAttempts}, wait=${config2.waitPerScroll}ms, stability=${config2.stabilityChecks}`);
    let maxHeight = 0;
    chrome.runtime.sendMessage({
      action: "PROGRESS",
    ...
    console.log(`[SahAI] Phase 1: Scrolling to bottom (${config2.bottomAttempts} attempts)...`);
    for (let i = 0; i < config2.bottomAttempts; i++) {
      ...
      console.log(`[SahAI] Bottom scroll ${i + 1}/${config2.bottomAttempts}: height ${currentHeight}px (max: ${maxHeight}px)`);
    ...
    console.log(`[SahAI] Phase 2: Scrolling to top (${config2.topAttempts} attempts)...`);
    chrome.runtime.sendMessage({
      action: "PROGRESS",
    ...
    for (let i = 0; i < config2.topAttempts; i++) {
      ...
      await new Promise((resolve) => setTimeout(resolve, config2.waitPerScroll));
      const currentHeight = container.scrollHeight;
      console.log(`[SahAI] Top scroll ${i + 1}/${config2.topAttempts}: height ${currentHeight}px (max: ${topMaxHeight}px)`);
      if (currentHeight === topMaxHeight) {
        topSameHeightCount++;
        if (topSameHeightCount >= config2.stabilityChecks) {
          console.log(`[SahAI] Top height stable for ${config2.stabilityChecks} checks - all oldest messages loaded`);
          break;
        }
      } else {
  ...
  async function extractFromMultiplePositions(adapter2) {
    ...
    console.log(`[SahAI] Using platform wait time: ${config2.parallelWait}ms`);
    ...
    for (const point of extractionPoints) {
      ...
      const pointPrompts = await adapter2.scrape();
  ...
  async function extractPrompts(source = "auto") {
    ...
    if ((source === "auto" || source === "dom") && adapter) {
      ...
      if ((platformName == null ? void 0 : platformName.toLowerCase()) === "lovable") {
        ...
        console.log(`[SahAI] Using fast DOM extraction for ${platformName}...`);
...
`;
  ...
  function findInputContainer() {
    ...
    if (platformName === "chatgpt") {
      const form = document.querySelector('form[class*="stretch"], form.w-full');
      if (form) {
        const innerDiv = form.querySelector('div[class*="rounded"]');
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    switch (message.action) {
      ...
      case "EXTRACT_PROMPTS": {
        ...
        isExtracting = true;
        const extractBtn = document.getElementById("pe-extract-btn");
