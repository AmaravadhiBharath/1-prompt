(function() {
  ...
  function getScrollConfig(platformName) {
    ...
    const config2 = configSet[normalizedName] || configSet[platformName || ""];
    if (config2) {
      console.log(
  ...
  function getConfigTier(platformName) {
  ...
  if (!isBridgeOnly) {
    let updateAdapter = function() {
    ...
    }, createExtractionResult = function(prompts) {
      const conversationId = getConversationId();
      return {
        platform: platformName || "unknown",
      ...
      return zone1;
    }, updateButtonLoading = function(button) {
    ...
    Capturing...
  `;
    }, updateButtonDone = function(button, originalText) {
      button.innerHTML = `
