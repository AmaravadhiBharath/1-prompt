...
import { DEFAULT_AI_CONFIG, loadAIConfiguration, validateAIConfig } from './ai-config';
...
export class DynamicConfigLoader {
  ...
  private async loadFromEnvironment(): Promise<Partial<AIConfiguration> | null> {
    try {
      ...
      if (envConfig.primary.provider !== DEFAULT_AI_CONFIG.primary.provider ||
  ...
  private async loadFromRemoteConfig(): Promise<Partial<AIConfiguration> | null> {
    try {
      ...
      const remoteConfigService = (await import('../services/remote-config')).RemoteConfigService.getInstance();
   ...
   */
  private mergeConfigs(base: AIConfiguration, override: Partial<AIConfiguration>): AIConfiguration {
    return {
   ...
   */
  async updateProvider(provider: AIProvider, model?: string, apiKey?: string): Promise<boolean> {
    try {
   ...
   */
  getCurrentProviderInfo(): { provider: AIProvider; model?: string; source: string } {
    return {
