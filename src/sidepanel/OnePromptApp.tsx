...
export default function OnePromptApp() {
    ...
    const [extractionSource, setExtractionSource] = useState<'auto' | 'dom' | 'keylog'>('auto');
    ...
    useEffect(() => {
        ...
        const onTabUpdated = (_tabId: number, _changeInfo: any, tab: chrome.tabs.Tab) => {
    ...
    useEffect(() => {
        const messageListener = async (msg: any) => {
            if (msg.action === 'EXTRACTION_RESULT' || msg.action === 'EXTRACTION_FROM_PAGE_RESULT') {
                ...
                // Calculate time
                const duration = startTimeRef.current ? parseFloat(((Date.now() - startTimeRef.current) / 1000).toFixed(1)) : 0;
    ...
    const handleCopy = async () => {
        ...
        const promptsToCopy = selectedPrompts.length > 0
            ? selectedPrompts.map(i => cleanContent(extractionResult?.prompts[i].content || '')).join('\n\n')
    ...
    const handlePin = async () => {
        ...
        const updated = history.map(item =>
            item.id === currentHistoryId ? { ...item, isPinned: newPinnedState } : item
        );
    ...
    const renderHeader = () => (
        <div className="kb-header-icons">
            {tier !== 'guest' && (
                <button className="kb-icon-button" onClick={loadHistory} title="History">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>
                </button>
            )}
            <button className="kb-icon-button" onClick={() => setShowPopup(!showPopup)} title="Profile">
                <svg width="24" height="24" viewBox="0 0 24 24" fill={user ? "currentColor" : "none"} stroke="currentColor" strokeWidth={user ? "0" : "2"}><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>
            </button>
        </div>
    );
    ...
    const renderUnsupported = () => (
        <div className="kb-screen-content">
            <h1 className="kb-title-large">This page<br />is not<br />supported</h1>
            <p className="kb-text-body" style={{ marginBottom: 16 }}>
                Please navigate to a <a href="#" onClick={() => setShowUpgradeModal(true)} style={{ color: 'var(--kb-primary-blue)', textDecoration: 'none', fontWeight: 600 }}>supported site</a> to use this extension.
            </p>
            <p className="kb-link-text">
                Try <a href="#" onClick={() => chrome.tabs.create({ url: 'https://chatgpt.com' })} style={{ color: 'inherit' }}>ChatGPT</a>, <a href="#" onClick={() => chrome.tabs.create({ url: 'https://claude.ai' })} style={{ color: 'inherit' }}>Claude</a>, or <a href="#" onClick={() => chrome.tabs.create({ url: 'https://chat.deepseek.com' })} style={{ color: 'inherit' }}>DeepSeek</a>.
            </p>
        </div>
    );
    ...
    const renderUnconnected = () => (
        <div className="kb-screen-content">
            <h1 className="kb-title-large">Connection<br />Lost</h1>
            <p className="kb-text-body">
                Handshake missing. Please refresh to continue.
            </p>
            <button className="kb-btn-refresh" onClick={() => chrome.tabs.query({ active: true, currentWindow: true }, tabs => tabs[0]?.id && chrome.tabs.reload(tabs[0].id))}>
                Refresh
            </button>
    ...
    const renderConnected = () => (
        <div className="kb-screen-content">
            <h1 className="kb-title-large">One Click<br />Away</h1>
            <p className="kb-text-body">
                Connected to {status.platform || 'AI Chat'}.<br />Choose a mode to begin.
            </p>

            <div className="kb-action-card" onClick={() => handleExtract('capture')}>
                <span className="kb-card-label">Capture</span>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14" /><path d="M12 5l7 7-7 7" /></svg>
            </div>
            ...
            {(tier === 'pro' || tier === 'infi' || tier === 'admin') && (
                <div style={{ width: '100%', marginTop: 24 }}>
                    ...
                    {showProSettings && (
                        <div style={{ marginTop: 12, background: 'rgba(0,0,0,0.03)', padding: 12, borderRadius: 8, fontSize: 12, textAlign: 'left' }}>
                            <div style={{ marginBottom: 8 }}>
                                <label style={{ display: 'block', fontWeight: 600, marginBottom: 4 }}>Extraction Mode</label>
                                <select
                            ...
                            <div style={{ borderTop: '1px solid rgba(0,0,0,0.05)', paddingTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
                                <div style={{ opacity: 0.6, display: 'flex', alignItems: 'center', gap: 6 }}>
                                    <span>🧠</span> Advanced Models
                                    <span style={{ fontSize: 10, background: '#eee', padding: '2px 4px', borderRadius: 4 }}>Coming Soon</span>
                                </div>
                                <div style={{ opacity: 0.6, display: 'flex', alignItems: 'center', gap: 6 }}>
                                    <span>💬</span> Priority Support
                                    <span style={{ fontSize: 10, background: '#eee', padding: '2px 4px', borderRadius: 4 }}>Active</span>
                                </div>
                            </div>
                        </div>
                    )}
    ...
    const renderResults = () => (
        <div className="kb-results-wrapper">
            <div className="kb-results-header">
                <button className="kb-back-circle" onClick={() => {
            ...
            <div className="kb-stats-bar">
                <div className="kb-stats-text" style={{ marginBottom: 0 }}>
                    Captured {extractionResult?.prompts.length || 0} prompts in {extractionTime?.toFixed(0) || liveTime.toFixed(0)} s
                </div>
                {tier !== 'guest' && (
                    <button
                    ...
                    >
                        <svg width="20" height="20" viewBox="0 0 24 24" fill={isPinned ? "currentColor" : "var(--kb-text-secondary)"} xmlns="http://www.w3.org/2000/svg" style={{ transform: 'rotate(45deg)' }}>
            ...
            <div className="kb-results-card">
                {mode === 'compile' && extractionResult?.summary ? (
                    <div className="kb-summary-section">
                        <div className="kb-summary-content">
                        ...
                        <div style={{ borderTop: '1px solid var(--kb-border)', marginTop: 16, paddingTop: 12 }}>
                            <button
                    ...
                    <div className="kb-results-scroll">
                        {extractionResult?.prompts.map((p, i) => (
                            ...
                            >
                                {selectedPrompts.includes(i) && <div className="kb-selection-dot" />}
                ...
                <div className="kb-card-footer">
                    {mode === 'compile' && extractionResult?.summary ? (
                        <button className="kb-footer-btn-primary" onClick={() => {
                        ...
                        selectedPrompts.length > 0 ? (
                            <>
                                <button className="kb-footer-btn-secondary" onClick={handleDelete}>
                                    Delete
                                </button>
            ...
            <div className="kb-disclaimer">
                Please verify content before use.
            </div>
        </div>
    );
    ...
    return (
        <div className="kb-app-container">
            ...
            {(showResults || viewingHistory) && (
                <div className="kb-header-icons">
                    {!viewingHistory && tier !== 'guest' && <button className="kb-icon-button" onClick={() => loadHistory()} title="History"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg></button>}
                    <button className="kb-icon-button" onClick={() => setShowPopup(!showPopup)} title="Profile"><svg width="24" height="24" viewBox="0 0 24 24" fill={user ? "currentColor" : "none"} stroke="currentColor" strokeWidth={user ? "0" : "2"}><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg></button>
                </div>
            )}

            {loading && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(255,255,255,0.8)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
                    <div className="kb-spinner" style={{ width: 32, height: 32, border: '3px solid #E5E7EB', borderTopColor: 'var(--kb-logo-vibrant)', borderRadius: '50%' }}></div>
                    <p style={{ marginTop: 16, color: 'var(--kb-text-secondary)', fontWeight: 500 }}>
                        {mode === 'compile' ? 'Compiling...' : 'Capturing...'} {liveTime.toFixed(1)}s
                    </p>
                </div>
            )}
            ...
            {viewingHistory ? (
                <div className="kb-history-overlay kb-fade-in">
                    <div className="kb-results-header">
                        <button className="kb-back-circle" onClick={() => setViewingHistory(false)}>
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M19 12H5" /><path d="M12 19l-7-7 7-7" /></svg>
                        </button>
                    ...
                    <div className="kb-history-filters" style={{ display: 'flex', gap: 8, padding: '0 16px 12px', overflowX: 'auto', scrollbarWidth: 'none' }}>
                        <button
                    ...
                    {tier === 'guest' ? (
                        <div className="kb-screen-content">
                            ...
                            <button className="kb-btn-primary" onClick={handleLogin}>Sign In</button>
                        </div>
                    ) : (
                            ...
                            {groupHistoryByDate(historyItems).map(group => (
                                <div key={group.label}>
                                    ...
                                    {group.items.map(item => (
                                        <div key={item.id} className="kb-history-card-premium" onClick={() => {
                                            setExtractionResult({
                                            ...
                                            <div className="kb-history-card-top">
                                                ...
                                                <div className="kb-history-meta">
                                                    {item.isPinned && (
                                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="var(--kb-logo-vibrant)" style={{ transform: 'rotate(45deg)' }}>
                                                            <path d="M16 12V4H8v8l-2 2v2h5.2v6h1.6v-6H18v-2l-2-2z" />
                                                        </svg>
                                                    )}
                                                    <span className="kb-history-date">{new Date(item.timestamp).toLocaleDateString()}</span>
                                                </div>
                                            </div>
            ...
            {showPopup && (
                <>
                    <div className="kb-popup-backdrop" onClick={() => setShowPopup(false)} />
                    <div className="kb-profile-menu kb-fade-in" onClick={(e) => e.stopPropagation()}>
                        {user ? (
                            <>
                                <div className="kb-menu-item" style={{ borderBottom: '1px solid #F3F4F6', marginBottom: 4, paddingBottom: 10 }}>
                                    <div style={{ fontWeight: 600 }}>{user.name || 'User'}</div>
                                    <div style={{ fontSize: 12, color: 'var(--kb-text-secondary)' }}>{tier === 'guest' ? 'Basic' : (tier === 'free' || tier === 'go' ? 'Go' : 'Pro')} Plan</div>
                                </div>
                                {(tier === 'free' || tier === 'go') && (
                                    <div className="kb-menu-item" style={{ color: 'var(--kb-logo-vibrant)' }} onClick={() => setShowUpgradeModal(true)}>
                                        <div style={{ fontWeight: 600 }}>Upgrade to PRO 🚀</div>
                                        <div style={{ fontSize: 11, color: '#60A5FA' }}>Unlimited Compiles & More</div>
                                    </div>
                                )}
                                <div className="kb-menu-item" style={{ color: '#EF4444', marginTop: 4 }} onClick={() => signOut()}>Sign Out</div>
                            </>
                        ) : (
                            <div className="kb-menu-item" onClick={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                                <div style={{ fontWeight: 600 }}>Login with Google</div>
                                <div style={{ fontSize: 11, color: 'var(--kb-text-secondary)' }}>Unlock Go Tier (Free!)</div>
                            </div>
                        )}
                    </div>
                </>
                ...
                <UpgradePrompt
                    ...
                    onUpgradeToPro={async () => {
                        ...
                        try {
                            ...
                            const { invalidateTierCache } = await import('../services/pricing');
